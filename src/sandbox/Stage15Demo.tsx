...
const STAGE14_BUNDLE_PATH = `${import.meta.env.BASE_URL}bundles/stage15.json`
...
function readInscriptions(world: WorldState): UID[] {
  const journal = world.entities[STAGE15_JOURNAL_ID]
...
export default function Stage14Demo() {
  ...
  const worldRef = useRef<WorldState>(createStage15World())
  ...
  const reset = async () => {
    if (busy) return
    worldRef.current = createStage15World()
    try {
      await bindRuntime()
      setTurn(0)
      applyChronicle([])
    ...
    <div className="algo-demo items-demo">
      ...
      <header className="algo-demo-head">
        ...
        <div className="items-read-row">
          ...
          {STAGE2_CHARACTERS.map((c) => (
            <button
              ...
              title={`Force ${c.name} to read the journal right now`}
            >
              {c.name} reads
            </button>
          ))}
        ...
        <p className="plan-tip dim">
          Step turns to watch characters greet and write in — or read — the journal
          naturally. Use the buttons above to force a character to read right now.
        </p>
      </header>
