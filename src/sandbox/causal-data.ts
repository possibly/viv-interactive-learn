...
export interface Storyline {
  ...
  nodes: StepRef[]
  edges: StorylineEdge[]
  convergesToStep?: number  // final step where this storyline feeds into a shared event
}

// Storyline nodes and edges are expressed in terms of the fixed step numbers
// from the template above.  getCharacters() resolves actual names for display.
export function getStorylines(chronicle: ChronicleEvent[]): Storyline[] {
  ...
  return [
    ...
    },
    {
      ...
      description: 'The note story is inscribed into a physical tome; when the tome is shown to another character and read, both the delivery action and the original note are causes of the reading.',
      method: 'inscription',
